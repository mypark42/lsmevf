...
#define call_orig(NAME, ...)                                                  \
	({                                                                    \
		typeof(lsmevf_##NAME) *orig =                                 \
			(typeof(lsmevf_##NAME) *)lsmevf_hooks[NAME].key.func; \
		orig(__VA_ARGS__);                                            \
	})
...
static int lsmevf_sb_mount(const char *dev_name, const struct path *path,
...
//LSM_HOOK(int, 0, move_mount, const struct path *from_path, const struct path *to_path)
static int lsmevf_move_mount(const struct path *from_path,
			     const struct path *to_path)
{
	...
	union event ev;
	ev.type = move_mount;
	ev.mount.move_mount.from_path = from_path;
	ev.mount.move_mount.to_path = to_path;
...
static void patch_lsm_static_call(struct lsmevf_hook *hook)
{
	...
	__static_call_update(lsm->key, lsm->trampoline,
			     hook->hook.lsm_func_addr);
...
#define LSMEVF_HOOK_INIT(HOOK)                                                 \
	do {                                                                   \
		...
		memcpy(&lsmevf_hooks[HOOK].key, last->key,                     \
		       sizeof(struct static_call_key));                        \
	} while (0);

int lsmevf_hook_init(void)
{
	lsmevf_static_calls_table =
		(typeof(lsmevf_static_calls_table))lsmevf_kallsyms_name(
			"static_calls_table");
